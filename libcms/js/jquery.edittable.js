...
(function ($, window, i) {
    ...
    $.fn.editTable = function (options) {
        ...
        function fillTableData(data) {
            ...
            if (s.headerCols || s.row_template) {
                ...
                for (a = 0; a < data[0].length; a += 1) {
                    console.log("displayAddDelButton = " + s.displayAddDelButton);
                    if(s.displayAddDelButton)
        ...
        $table.on('click', '.delrow', function () {
            ...
            if($(this).parent().parent().children('td').eq(7).children('input').val() !== ""){
                alert("已预订的行在 '预订查询' 功能中删除!");
                return;
            }
            ...
            if ($(this).hasClass('disabled')) {
                // console.log($(this).parent().parent().children('td').eq(6).children('input').val());
                $(this).parent().parent().children('td').each(function (index, o) {
                    if($(o).children('input').val() !== undefined){
                        $(o).children('input').val("");
                    }
                });
                return false;
            }
