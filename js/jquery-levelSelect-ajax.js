    ...
    $.openLayer = function(p){
        ...
        var fs = {
            init_Container : function(){	//初始化头部和内容容器
                ...
                //头部
                var htmlDiv = "<div id='heads'><div id='headdiv'><span id='stitle'>" + TITLE + "</span><span id='close'>" + CLOSE + "</span></div>";
                //内容容器创建部分
            ...
            add_data : function(targetid){					//添加数据到容器，添加事件，初始化下一层次容器
                ...
                if ((data == null || data == "") &&  url != ""){
                    ...
                    $.ajax({
                        type : "get",						//post方式
                        url : backServerUrl + url,							//ajax查询url
                        ...
                        success : function (d) {			//ajax请求成功后返回数据
                            ...
                            // data = {"0":sdata};
                            data = sdata.substring(-1,sdata.length - 1);
                ...
                var inspan_width = (/msie/.test(navigator.userAgent.toLowerCase()))?1:3;								//内部文字和checkbox之间的距离
            ...
            init_event : function(){		//绑定已选择框中checkbox的事件，确定，取消事件响应
                ...
                // });
                $(document).on("click","#selArea :input",function(){     //修改成这样的写法
                    $(this).parent().remove();
                    $("#container > div").find(":input[value="+this.value+"]").attr("checked",false);
                });
            ...
            init_style : function() {	//初始化css
                ...
                css.push("#headdiv {color:white;background-color:cadetblue;font-size:13px;height:25px;margin:1px;" +aotu+"}");
                css.push("#stitle {line-height:30px;padding-left:20px;float:left;}");
        ...
        if (param.dragEnable) {		//允许鼠标拖动
            ...
            popupDiv.mousedown(function(e){
                ...
                if(_move){
                    var sx=e.pageX-_x;//移动时根据鼠标位置计算控件左上角的绝对位置
                    var sy=e.pageY-_y;
                    popupDiv.css({top:sy,left:sx});//控件新位置
...
_cache ={"0":"0100:原著,0200:著,0300:编著,0500:主编,0600:编,0700:改编,0800:口述,0900:整理,1000:绘,1100:记录,1200:缩写,1300:译"};
